...
void RedSocial::cargarUsuarios(const char* nombreArchivo) {
    ...
    qsort(usuarios, cantUsuarios, sizeof(UsuarioEstandard), compararUsuarios);
}
...
UsuarioEstandard& RedSocial::operator[](int indice) const {
...
UsuarioEstandard& RedSocial::operator[](const char* cuenta) const {
...
UsuarioEstandard& RedSocial::at(int indice) const {
    return (*this)[indice];
}

RedSocial& RedSocial::operator+=(const UsuarioEstandard& usuario) {
...
void RedSocial::incrementar() {
    capacidad += 5;
    if (usuarios == nullptr) {
        usuarios = new UsuarioEstandard[capacidad];
    }
    else {
        UsuarioEstandard* aux = new UsuarioEstandard[capacidad];
        for (int i = 0; i < cantUsuarios; i++) {
            aux[i] = usuarios[i];
        }
        delete[] usuarios;
...
int compararUsuarios(const void* a, const void* b) {
    UsuarioEstandard usuarioA = *(UsuarioEstandard*)a;
    UsuarioEstandard usuarioB = *(UsuarioEstandard*)b;
