...
Así, la saga de La Guerra de las Galaxias sigue viva en los corazones de todos aquellos que creen en la lucha entre la luz y la oscuridad, en la fuerza que mueve el universo y en la eterna esperanza de que, al final, el bien prevalecerá.)";
    ...
    map<string, int> bagOfWords; // Entradas (Entries) - Entry
    // Un Entry es una pareja clave-valor (key-value pair)
    ...
    if (pos != string::npos) {
        cout << endl;
        cout << "La palabra contiene 'fuerza', en la posición: " << pos << endl;
    }

    string ejemplo2 = "el lado oscuro de la fuerza";
    size_t pos1 = ejemplo1.find("fuerza");
    string sub = ejemplo2.substr(pos1, 6);
    cout << "\nSubcadena de '" << ejemplo2 << "': " << sub << endl;

    // Ejemplo de substr
    string ejemplo3 = "galaxias";
    string sub1 = ejemplo3.substr(0, 4);  // "gala"
    cout << "\nSubcadena de '" << ejemplo2 << "': " << sub1 << endl;
