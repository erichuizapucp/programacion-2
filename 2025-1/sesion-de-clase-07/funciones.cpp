...
void cargarMesasExactas(ifstream& archivo, int**& mesas) {
    ...
    while (archivo >> codigo) {
        ...
        buffer[numMesas * 2 + CODIGO] = codigo;
        buffer[numMesas * 2 + CAPACIDAD] = capacidad;
    ...
    for (int i = 0; i < numMesas; i++) {
        codigos[i] = buffer[i * 2 + CODIGO];
        capacidades[i] = buffer[i * 2 + CAPACIDAD];
    }
